...
export class FuncionesEstDescpService {

  constructor(private mathService: MathService) { }

  media(data: number[]): number {
    return this.mathService.redondear(this.mathService.dividir(this.mathService.sumarDatos(data), this.mathService.longitud(data)), 2);
  }
  ...
  cuartil_1(data: number[]): number {
    const mitadIzquierda = data.filter(x => x <= this.mediana(data)).slice(0, this.mathService.dividir(this.mathService.longitud(data), 2));
  ...
  cuartil_3(data: number[]): number {
    const mitadDerecha = data.filter(x => x >= this.mediana(data)).slice(this.mathService.dividir(this.mathService.longitud(data), 2) * (-1));
  ...
  baseInferior(data: number[]): number {
    const Q1 = this.cuartil_1(data);
    const RI = this.rangoIntercuartilico(data);
  
    return Q1 - 1.5 * RI;
  }
  
  baseSuperior(data: number[]): number {
    const Q3 = this.cuartil_3(data);
    const RI = this.rangoIntercuartilico(data);
  ...
  contarRepeticiones(data: number[]): Object {
    ...
    // set.forEach(x => {
    //   resultadoConteo[x.toString()] = data.filter(y => y === x).length;
    // });
